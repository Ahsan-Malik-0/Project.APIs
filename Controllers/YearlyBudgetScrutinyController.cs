...
namespace Project.APIs.Controllers
{
    ...
    public class YearlyBudgetScrutinyController(YearlyBudgetScrutinyService scrutinyService, YearlyBudgetService yearlyBudgetService) : ControllerBase
    {
        ...
        [HttpPost("approveYearlyBudget/{yearlyBudgetId}")]
        public async Task<IActionResult> ApproveYearlyBudget(Guid yearlyBudgetId, [FromBody] ApproveYearlyBudgetDto approvalDetails)
        {
            try
            {
                await yearlyBudgetService.ApproveYearlyBudget(yearlyBudgetId, approvalDetails);
                return Ok();
            }
            catch (NotFoundException ex)
            {
